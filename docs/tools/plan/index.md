...
Plans are stored as JSON files in the docker-agent data directory (`~/.local/share/cagent/plans/` by default). All agents that share a process serialize on a single mutex so concurrent reads and writes are safe. Writes are atomic (temp file + rename), so a reader never observes partial content.
